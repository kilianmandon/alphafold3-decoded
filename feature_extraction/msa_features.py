...
class HotfixDuplicateRowIfSingleMSA(Transform):
    """
    Due to how AlphaFold manages paired and unpaired MSAs, if there is no paired MSA, the first line of the unpaired MSA (which is just the sequence itself) is duplicated. 
    """
...
class HotfixAF3LigandAsGap(Transform):
    def forward(self, data):
        ...
        data['msa_features']['msa'][0, ligand_inds] = PROTEIN_TO_ID['-']
        return data
...
class CalculateMSAFeatures(Transform):
    def __init__(self, max_msa_sequences, msa_trunc_count, n_cycle, protein_msa_dirs=None, rna_msa_dirs=None, msa_shuffle_orders=None):
        self.transforms = Compose([
            ...
            EncodeMSAFeatures(msa_trunc_count, msa_shuffle_orders, n_cycle)
        ])
