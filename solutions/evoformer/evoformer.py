...
class TemplateEmbedder(nn.Module):
    ...
    def forward(self, batch: Batch, z: torch.Tensor):
        ...
        dummy_a = torch.zeros(batch_shape+(n_token, n_token, n_templates, self.c_in), device=device, dtype=torch.float32)
...
class MSAModule(nn.Module):
    def __init__(self, c_m, c_z, msa_feat_dim, target_feat_dim, config: MSAModuleConfig):
        super().__init__()
        self.linear_m = nn.Linear(msa_feat_dim, c_m, bias=False)
        self.linear_s = nn.Linear(target_feat_dim, c_m, bias=False)
