...
class EncodeMSA(Transform):
    def __init__(self):
        ...
        for i_msa, code in MSA_INTEGER_TO_THREE_LETTER.items():
            self.lookup_table[i_msa] = AF3_TOKENS_MAP.get(
                code, AF3_TOKENS_MAP[UNKNOWN_AA]
            )
    ...
    def forward(self, data):
        ...
        for chain_id, msa_data in data["polymer_msas_by_chain_id"].items():
            data["polymer_msas_by_chain_id"][chain_id]["msa"] = self.lookup_table[
                msa_data["msa"]
            ]
...
class ConcatMSAs(Transform):
    ...
    def forward(self, data):
        ...
        if len(polymer_msas) == 0:
            ...
            max_msa_size = max(
                msa_data["msa"].shape[0] for msa_data in polymer_msas.values()
            )

        padded_token_count = data["token_features"].token_count

        full_msa = np.zeros(
            (self.max_msa_sequences, padded_token_count), dtype=np.int64
        )
        deletion_count = np.zeros(
            (self.max_msa_sequences, padded_token_count), dtype=np.float32
        )
        full_msa_mask = np.zeros(
            (self.max_msa_sequences, padded_token_count), dtype=np.float32
        )
        individual_msa_mask = np.zeros(
            (self.max_msa_sequences, padded_token_count), dtype=np.float32
        )
        ...
        for chain_start, chain_end in zip(chain_starts, chain_ends):
            ...
            if chain_id in polymer_msas:
                ...
                deletion_count[:msa_size, residue_starts] = polymer_msas[chain_id][
                    "ins"
                ]
                individual_msa_mask[:msa_size, residue_starts] = ~polymer_msas[
                    chain_id
                ]["msa_is_padded_mask"]
...
class AssembleMSAFeatures(Transform):
    ...
    def calculate_target_feat(self, restype, profile, deletion_mean):
        ...
        target_feat = np.concatenate(
            (restype_one_hot, profile, deletion_mean[..., None]), axis=-1
        )
    ...
    def sample_msa_features(self, base_msa_feat, base_msa_mask):
        ...
        if self.msa_shuffle_orders is None:
            ...
            scores = (
                odds + torch.distributions.Gumbel(0, 1).sample(scores_shape).numpy()
            )
    ...
    def forward(self, data):
        ...
        has_deletion = np.clip(deletion_count, a_min=0, a_max=1, dtype=np.float32)[
            ..., None
        ]

        full_msa_feat = np.concatenate(
            [msa_one_hot, has_deletion, deletion_value], axis=-1
        )
