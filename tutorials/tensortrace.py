...
class TensorTrace:
    def __init__(self, path: str, mode: str, framework='numpy'):
        ...
        self.loading_index: dict[str, int] = {}

        self.context = {}

    def reset_loading_index(self):
        self.loading_index = {}
    ...
    def compare(self, value: (Any|dict[str, Any]), name: str, processing:Optional[dict|list|Any]=None, input_processing:Optional[dict|list]=None, use_mask: (dict|bool)=True):
        ...
        for k, v1 in value.items():
            ...
            di, da, dr = diff(v1, v2)
            da_max = da[tuple(np.moveaxis(di, -1, 0))].max() if di.size > 0 else 0
            dr_max = dr[tuple(np.moveaxis(di, -1, 0))].max() if di.size > 0 else 0
...
def log_or_compare(value: (Any|dict[str, Any]), name: str, use_mask: (dict|bool)=True):
    if current_trace().mode == 'write':
        log(value, name)
        return True
    else:
        return compare(value, name, use_mask=use_mask)
...
def reset_loading_index():
    current_trace().reset_loading_index()
