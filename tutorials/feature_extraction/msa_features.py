...
class HotfixDuplicateRowIfSingleMSA(Transform):
    """
    Due to how AlphaFold manages paired and unpaired MSAs, if there is no paired MSA, the first line of the unpaired MSA (which is just the sequence itself) is duplicated. This transform patches the data in this case by duplicating the first row.
    """
    ...
    def forward(self, data):
        ...
        if len(msa_hashes) == 1:
            ...
            # This is because duplication happens during the AF3 MSA pairing procedure, and profile and deletion_mean are calculated 
            # on the unpaired MSA
...
class HotfixAF3LigandAsGap(Transform):
    """
    In the MSA encoding, AF3 actually doesn't use the ligand's restype token but the GAP token (this affects the first row of the MSA). This transform mirrors that.
    """
    def forward(self, data):
...
class ConcatMSAs(Transform):
    ...
    def forward(self, data):
        ...
        Populates data['msa_features'] with the following features:
            ...
            - deletion_count: Array of shape (max_msa_sequences, n_tokens), containing the count of deletions (insertions in Atomworks terminology) before each position for each MSA sequence.
            - full_msa_mask: Array of shape (max_msa_sequences, n_tokens), with the full block of shape (max_msa_size, unpadded_token_count) set to 1, ignoring individual msa sizes. max_msa_size is the maximum size of the MSAs across all chains.
            - individual_msa_mask: Array of shape (max_msa_sequences, n_tokens), with the valid positions for each MSA sequence set to 1. Used for calculating the profile and deletion_mean features.
        """
        ...
        TODO: Implement MSA concatenation, consisting of the following steps:
            - Set the full block (max_msa_size, unpadded_token_count) to the GAP encoding (AF3_TOKENS_MAP['<G>']) and the corresponding full_msa_mask block to 1. Here, max_msa_size is the maximum size of the MSAs across all chains, or 1 if no MSAs are present.
            - Set the first row of the full MSA to the token_features restype, and update the first row of individual_msa_mask accordingly.
            - Plugging the actual MSAs into the full MSA is a bit tricky, because of the fact that some residues in the MSA might be atomized (in which case there are more tokens in the feature than residues in the MSA). In this case, only the first token of the residue (which corresponds to the get_residue_starts(atom_array) of the atom array) should get the MSA values. For the individual MSAs, you can create an integer index and use that to populate msa, deletion_count, and individual_msa_mask, e.g. msa[:msa_size, residue_starts_for_chain_id] = polymer_msas[chain_id]['msa']. The corresponding Atomworks names are polymer_msas[chain_id]['msa'], polymer_msas[chain_id]['ins'], and ~polymer_msas[chain_id]['msa_is_padded_mask'] respectively (not the negation for the latter, msa_is_padded_mask is the negation of the actual mask).
...
class AssembleMSAFeatures(Transform):
    def __init__(self, msa_trunc_count, n_recycling_iterations, msa_shuffle_orders=None):
    ...
    def calculate_target_feat(self, restype, profile, deletion_mean):
        ...
        Args:
            ...
            profile (Array): Array of shape (n_tokens, 32), containing the residue profile over the MSA for each position.
            deletion_mean (Array): Array of shape (n_tokens,), containig the mean deletion count over the MSA for each position.

        Returns:
            Array: target_feat of shape (n_tokens, 65), stacking the one-hot encoded restype, the profile, and deletion_mean.
        """
    ...
    def sample_msa_features(self, base_msa_feat, base_msa_mask):
        """
        Samples a random subset of msa_trunc_count rows from base_msa_feat and base_msa_mask. The subset is sampled uniformly, except for preferring rows that have at least one valid entry.
        ...
        Returns:
            tuple: (msa_feat, msa_mask) of shapes (msa_trunc_count, n_tokens, msa_feat_dim, n_cycle) and (msa_trunc_count, n_tokens, n_cycle) respectively, containing the sampled MSA features and masks.
        """
        ...
        """
        TODO: Implement the sampling procedure. For the results to match with our test cases, you have to implement our uniform-order strategy, which goes as follows:
          - Calculate a 1/0 mask of which rows have at least one valid entry (e.g. using np.sum and np.clip)
          - Sample a random score using torch.distributions.Gumpel(0, 1) and add a large positive constant (e.g. 1e2) to rows without a valid entry. To match  the test results, sample all scores, e.g. for (n_cycle, max_msa_sequences), at once. 
          - argsort the scores in ascending order to get a shuffle order. Truncate that to msa_trunc_count to get the sampled row indices, and use those to create the sampled msa_feat and msa_mask (of shape (n_cycle, msa_trunc_count, n_tokens, [msa_feat_dim]))
    ...
    def forward(self, data):
        ...
        TODO: Build the MSAFeatures object, through the following steps:
            - Calculate profile and deletion_mean, using utils.masked_mean (on deletion_count and a one-hot encoding of msa, with 32 classes). These are based on individual_mask (e.g. the real mask), not full_msa_mask (the larger block mask)
...
class CalculateMSAFeatures(Transform):
    def __init__(self, max_msa_sequences, msa_trunc_count, n_cycle, protein_msa_dirs=None, rna_msa_dirs=None, msa_shuffle_orders=None):

        transforms = []

        """
        TODO: Assemble the MSA feature pipeline, consisting of the following steps:
          - LoadPolymerMSAs (Atomworks Transform): Loads MSAs for each polymer chain and stores it in the annotation data['polymer_msas_by_chain_id']. Set protein_msa_dirs, rna_msa_dirs, max_msa_sequences, put use_paths_in_chain_info=True (we specify the MSA paths in the chain info in load_alphafold_input)
          - HotfixDuplicateRowIfSingleMSA (Transform defined above): If there is only a single MSA, it duplicates the first row of that, to match AF3's handling.
          - EncodeMSA (Transform defined above): Replaces the atomworks-custom integer encoding of the MSA with the AF3 encoding.
          - ConcatMSAs (Transform defined above): Concatenates the explicit MSAs and puts their token_features.restype for the remaining features.
