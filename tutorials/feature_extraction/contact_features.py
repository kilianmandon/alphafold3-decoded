...
class CalculateContactMatrix(Transform):
    """
    Calculates the contact matrix of shape (n_tokens n_tokens 1). The contact matrix is a binary matrix that has ones exactly at indices i<j where either i or j is atomized (i.e. a ligand, or a modified residue), and there is a bond between any atom in token i and any atom in token j.
    """
    def forward(self, data):
        ...
        Hints: 
            ...
            - atom_array.atomize is a boolean array of shape (n_atom,) that indicates whether the atoms belong to an atomized token or not. This can be indexed with the token starts to get a boolean array of shape (n_tokens). Note that you might need to pad this array to the padded token count, since len(get_token_starts(atom_array)) is the unpadded token count.
