...
def collate_batch(batch: list[dataclass]|list[torch.Tensor]|list[dict], drop_unconvertible_entries=False) -> dataclass:
    """
    Recursively collates a list of nested structures of dataclasses, dicts, and tensors into a single nestex structure where the tensors are stacked along a new batch dimension. The tensors in the batch are padded to the maximum shape along each dimension to ensure they can be stacked together.
    ...
    if is_dataclass(first):
        field_dict = {f.name: collate_batch([getattr(b, f.name) for b in batch], drop_unconvertible_entries=drop_unconvertible_entries) for f in fields(first)}
        return type(first)(**field_dict)
    
    if isinstance(first, dict):
        field_dict = {k: collate_batch([b[k] for b in batch], drop_unconvertible_entries=drop_unconvertible_entries) for k in first.keys()}
        return field_dict
...
def custom_af3_pipeline(config: Config, msa_shuffle_orders=None, is_inference=True) -> Transform:
    ...
    Args:
        config: Config object for the whole AlphaFold model. In particular, the data pipeline needs to be aware of the number of recycling iterations, and choices for the msa truncation per iteration, 
        msa_shuffle_orders: An optional tensor of shape (n_recycling_iterations, n_msa_sequences) containing a deterministic shuffle order for the MSA sequences. If not provided, random shuffling will be performed for each recycling iteration. This can ensure reproducibility for testing.
        is_inference: Whether the pipeline is used for inference or training. For training, an additional random cropping step is added after atomization.
    """
    ...
    """
    TODO: Initialize a list of the transforms RemoveHydrogens, HotfixDropSaccharideO1, AddGlobalAtomIdAnnotation, AtomizeByCCDName (atomizing by default, except if they are part of STANDARD_AA, STANDARD_RNA, or STANDARD_DNA), and the four AF3-specific transforms CalculateTokenFeatures, CalculateRefStructFeatures, CalculateMSAFeatures, and CalculateContactMatrix (in this order), which we implement in the other files. Finally, append BuildBatch to assemble the dict into a Batch object. The transforms fulfill the following tasks:
    ...
    - AddGlobalAtomIdAnnotation: Adds range(N) as a global atom id annotation. We don't need that, but ourselves, but the Atomworks Cropping transforms for training require the transform. 
    - AtomizeByCCDName: Adds an 'atomize' flag to the input AtomArray, that is true for atoms belonging to ligands or modified residues. We use this flag in ConcatMSAs and CalculateContactMatrix. 
