...
# 3. **Reference Features**: These are atom-level features and encode information on the atom's element type, charge, and so on.
# 
# Additionally, there is the bond matrix (encoding which tokens are covalently bonded to which other tokens) and template features (which we will leave out, we are doing template-free prediction).
# 
...
ttr.TensorTrace('data/tensortraces/feature_extraction', mode='read', framework='numpy').start_trace()
...
# To warm up, we will start by implementing a few utility functions and the contact_matrix. Afterward, we are going through token features, reference features, and MSA features in that order. 
# 
...
if success: print('pad_to_shape tests passed.')
...
if success: print('round_down_to tests passed.')
...
print('round_to_bucket tests passed.')

transform = Compose(base_transforms + [CalculateBondMatrix()])


success = True
for inputs in test_inputs_pipeline:
    bond_matrix = transform(inputs)['bond_matrix']
    success = success and ttr.log_or_compare(bond_matrix, 'bond_matrix')

if success: print('Bond matrix tests passed.')
...
if success: print('Restype encoding tests passed.')
...
if success: print('Token feature calculation tests passed.')


# ## Reference Features
# Reference features are atom-level features, meaning they have shape (n_atoms,) instead of (n_tokens,). Here, just as n_tokens is padded to certain bucket sizes and not the real number of tokens, n_atoms is not the real number of atoms, but n_tokens * 24 (24 is the fixed number of atoms per token that AlphaFold users). We will refer to the real number of atoms as the unpadded atom count. However, during feature construction, the docstrings generally use n_atoms for the unpadded atom count for simplicity. Padding is done at the end of the reference feature construction transform.
# 
# The reference features themselves are quite simple to construct, most of them can be directly obtained from the atom_array without any processing. The exception is `positions`, which require the computation of a reference conformer for the residues and ligands (a proposed 3D structure of the molecule, computed using chemistry software). 
# 
# Aside from that, the main challenge in implementing reference features is allowing for conversion between a so-called atom-layout (having the features in shape (n_atoms,)) and a token-layout (having the features in shape (n_tokens, 24)). This is a bit tricky, because in the atom-layout, the atoms are dense (e.g. token_1_atom_1, token_1_atom_2, token_2_atom_1, ..., if the first token has only two atoms), while the token-layout would be sparse ([token_1_atom_1, token_1_atom_2, pad, pad, pad, ...], [token_2_atom_1, ...], ...). Because of this, the conversion is not a simple reshape. But it's not super complex either.

# We will start with the computation of the features. In `reference_features.py`, calculate the whole class `CalculateReferenceFeatures` (e.g. the methods `calculate_ref_positions`, `prep_atom_name_chars` for encoding the string atom names into integers, and the main `forward` method). After you are done, run the following cell to test your implementation. Note that this is below the dataclass `ReferenceFeatures`, we will implement that afterwards.

# In[ ]:


from feature_extraction.reference_features import CalculateReferenceFeatures
import numpy as np
...
calc_ref = CalculateReferenceFeatures()
...
if success: print('Reference feature calculation (without unknown entries) tests passed.')
...
if success: print('Reference feature calculation (with unknown entries) tests passed.')


# Now, we will complete the reference features by implementing the conversion between atom-layout and token-layout, e.g. by implementing the methods `to_token_layout` and `to_atom_layout`, as well as the two helper methods `token_layout_ref_mask` and `patch_atom_dimension` in the class `ReferenceFeatures`. The basic idea is this: We already have the mask for the reference features in atom-layout, e.g. of shape `(**batch_shape, n_atoms)`. Now, we can just build an equivalent mask in token-layout, of shape `(**batch_shape, n_tokens, 24)`, based on the info how many atoms are present in each token, which we do in `token_layout_ref_mask`. Then, we can do feature conversion as `feat = zeros(out_shape); feat[token_layout_ref_mask] = feat_atom_layout[atom_layout_ref_mask]` and vice versa. The method `patch_atom_dimension` is simply utility for the case that we want to do token-layout -> atom-layout conversion, but the input feature doesn't have an atom dimension and simply requires broadcasting along that dimension.
# 
# Implement these methods in `reference_features.py`, and test your implementation by running the following cell. Note that the layout conversions need to support batch dimensions (in contrast to the feature extraction pipeline, which only works on individual samples).

# In[ ]:


from feature_extraction.reference_features import CalculateReferenceFeatures, ReferenceFeatures
from feature_extraction.feature_extraction import collate_batch


ttr.reset_loading_index()
np.random.seed(0)
transform = Compose(base_transforms + [CalculateTokenFeatures(), CalculateReferenceFeatures()])
...
for i, test_data in enumerate(test_data_individual):
    reference_features: ReferenceFeatures = test_data['reference_features']
    to_atom = reference_features.to_atom_layout(test_token_layout[i], has_atom_dimension=True)
    to_atom_with_feats = reference_features.to_atom_layout(test_token_layout_with_feats[i], has_atom_dimension=True)
    to_atom_no_atom_dim = reference_features.to_atom_layout(test_token_layout_no_atoms[i], has_atom_dimension=False)
    to_token = reference_features.to_token_layout(test_atom_layout[i])
...
to_atom_with_feats_batch = reference_features_batch.to_atom_layout(collate_batch(test_token_layout_with_feats), has_atom_dimension=True)
to_atom_no_atom_dim_batch = reference_features_batch.to_atom_layout(collate_batch(test_token_layout_no_atoms), has_atom_dimension=False)
to_token_batch = reference_features_batch.to_token_layout(collate_batch(test_atom_layout))
...
print('Batch to_atom and to_token tests passed.')
...
if success: print('MSA encoding tests passed.')
...
if success: print('MSA feature concatenation tests passed.')


# The steps 3 and 4 are both done in the same transform `BuildMSAFeatures`. Concretely, it creates two features, the target_feat (which does not include an MSA dimension) and the msa_feat (which includes an MSA dimension), then subsamples this msa_feat. Implement the transform and run the following cell to test your implementation.

# In[ ]:


from feature_extraction.msa_features import EncodeMSA, ConcatMSAs, BuildMSAFeatures
from atomworks.ml.transforms.msa.msa import LoadPolymerMSAs
from config import Config
import torch
...
transform = Compose(base_transforms + [
    ...
    EncodeMSA(),
    ConcatMSAs(max_msa_sequences=config.featurization_config.max_msa_sequences),
    BuildMSAFeatures(config.featurization_config.msa_trunc_count, config.global_config.n_cycle)
    ])
...
if success: print('MSA feature assembly tests passed.')


# Lastly, the transform `BuildMSAFeaturePipeline` simply joines the previous transforms together into one single transform. Note that it also includes two transforms you didn't implement yourselves, `HotfixDuplicateRowIfSingleMSA` and `HotfixAF3LigandAsGap`. These account for some perks in AF3 feature encoding that might not be desirable for a general feature extraction pipeline, and are solely included to mirror AF3 exactly (so that the AF3 weights can be used for the model). 
# 
# Implement `BuildMSAFeaturePipeline` and run the following cell to test your implementation.

# In[ ]:


from feature_extraction.msa_features import BuildMSAFeaturePipeline
from atomworks.ml.transforms.msa.msa import LoadPolymerMSAs
from config import Config
import torch
...
transform = Compose(base_transforms + [
    CalculateTokenFeatures(), 
    CalculateReferenceFeatures(), 
    BuildMSAFeaturePipeline(config.featurization_config.max_msa_sequences, config.featurization_config.msa_trunc_count, config.global_config.n_cycle)
    ])
...
if success: print('Full MSA feature calculation tests passed.')
...
if success: print('Full feature extraction pipeline tests passed.')
